...
fail-fast provider validation (`app.llm_providers.factory`) *before* any test
module imports `app.main` (which triggers `create_app()` at module import
time). This keeps the test suite independent of whatever a developer's local
`.env` happens to contain, and makes it reproducible in CI where no `.env`
file exists at all.
