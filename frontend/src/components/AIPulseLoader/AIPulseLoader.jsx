 ...
 *
 * @param {{ label?: string, size?: 'sm' | 'md' | 'lg', className?: string }} props
 ...
 *   `size="lg"` (16px dots) matches the `AssistantStatusIndicator` spec in
 *   `04-chat-ui.md` §3 ("3-dot varian, 16px").
 */
export function AIPulseLoader({ label = 'Memproses...', size = 'md', className = '' }) {
  const prefersReducedMotion = usePrefersReducedMotion();
  const dotSize = size === 'sm' ? styles.dotSm : size === 'lg' ? styles.dotLg : '';
